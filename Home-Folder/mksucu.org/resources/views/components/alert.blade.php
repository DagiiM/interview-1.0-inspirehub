...
.flash-card{
        ...
        width:var(--_width);
        position:absolute;
        bottom:calc(var(--nav-height)*0.2);
        right:10%;
        padding:2px;
        border-radius:10px;
        display:flex;
...
.flash-card > .icon:nth-last-child(1){
   padding:2px;
   border:1px solid transparent;
}
.flash-card > .icon:nth-last-child(1):hover{
   padding:2px;
   cursor:pointer;
...
<svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
  ...
  <symbol id="danger" fill="currentColor" viewBox="0 0 16 16">
    ...
    <article class="flash-card alert-{{ $type }}" aria-live="polite" aria-atomic="true">
    <x-icon type="{{ $type }}" icon="{{ $type }}"/>
    <section class="flash-card-body">
       <strong class="flash-message">{{ $message }}</strong>
      </section>
