...
@if($data->count()>0)
  ...
  @foreach($data as $ebook)
    <tr>
        ...
        <td>{{$ebook->subject}}</td>
        <td>
              <a href="{{route('ebooks.libraries.index',['ebook'=>$ebook])}}">
                Library
            </a>
  ...
  <!-- Action Button -->
         <x-action-button view-link="" edit-link="{{route('ebooks.edit',$ebook)}}" delete-link="{{route('ebooks.destroy',['ebook'=>$ebook])}}">    
         </x-action-button>
